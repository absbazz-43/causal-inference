...
gen_dat <- function(n = 1000){
  ...
  #     sw_a = pa*wa
  #   ) %>% dplyr::select(-c(.fitted, pa))
  # 
  # 
  
  
  model2 <- qda(A ~ L1 + L2, data = data, ntree = 500, importance = TRUE)
  
  predT <- predict(model2, data, type = "class")
  predTrain = ifelse(predT$class==1,predT$posterior[,2],(1-predT$posterior[,1]))
  ...
  xgb.model <- train(
    ...
    trControl = trainControl("cv",p=0.8, number = 50)
  )
...
res <- res %>% 
  mutate(
    sample = rep(c(200, 500), each = 1),
    par = rep(-0.12,3)
  ) %>% 
  dplyr::select(sample, par, everything())
